...
if (codigoerror == 715) { # !dbIsValid (con)) {
   ...
   #stop ("Conexion DB invalida")
    ...
    if (tipo_carga == 'edocta') {
       result_carga = carga_edocta (anio,tipo_carga,varchivo1,con_bd)
       if (result_carga == 0) {
    ...
    if (result_carga == 0) {
        con_error = FALSE
        result_edo = crear_idx_edocta (anio,quincena,tipo_carga,vuser,varchivo1,con_bd,con_error) 
        codigoerror = result_edo$codigoerror
    } else {
        con_error = TRUE
        result_edo = crear_idx_edocta (anio,quincena,tipo_carga,vuser,varchivo1,con_bd,con_error)  
        codigoerror = result_edo$codigoerror       
